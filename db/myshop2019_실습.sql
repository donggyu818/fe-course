...
-- Q01) 전체 결제 금액이 8,500,000 이상인 주문의 주문번호, 고객명, 사원명, 주문일시, 전체금액을 조회하세요.
select * from customer;
select * from order_header;
select * from employee;
select oh.order_id, c.customer_name, e.employee_name, oh.order_date, format(oh.total_due, 0) as total_due
